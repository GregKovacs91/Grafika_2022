...
void init_pong(Pong* pong, int width, int height)
{
    ...
    init_ball(&(pong->ball), width/2, width/2);
}
...
{
    if (((pong->ball.x - pong->ball.radius < 50) && (pong->ball.x - pong->ball.radius > 0))&&(pong->ball.y>pong->left_pad.y-50 && pong->ball.y<pong->left_pad.y+180)) {
        pong->ball.x = pong->ball.radius + 50;
        pong->ball.speed_x = (pong->ball.speed_x*(-1)+20);
		pong->ball.speed_y +=20;
    }
    if (((pong->ball.x + pong->ball.radius > pong->width - 50) && (pong->ball.x + pong->ball.radius < pong->width)) &&(pong->ball.y>pong->right_pad.y -50 && pong->ball.y<pong->right_pad.y+180)) {
	...
	if(pong->ball.x - pong->ball.radius<0){
		pong->right_pad.points++;
		printf("Points: %d | %d\n",pong->left_pad.points,pong->right_pad.points);
		pong->ball.x=pong->width/2;
		pong->ball.speed_x=250;
		pong->ball.speed_y=250;
	}else if(pong->ball.x + pong->ball.radius > pong->width){
		pong->left_pad.points++;
		printf("Points: %d | %d\n",pong->left_pad.points,pong->right_pad.points);
		pong->ball.x=pong->width/2;
		pong->ball.speed_x=250;
		pong->ball.speed_y=250;
